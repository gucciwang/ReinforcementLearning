...
class A3C_Worker():
    ...
    # Train on weights
    def train(self, sess, thread_manager, saver=None, max_episode_length=None, discount_factor=None):
        ...
        with sess.as_default(), sess.graph.as_default():
            ...
            for _ in range(BATCH_SIZE):
                ...
                action = np.random.choice(action_indexes,
                                          p=policy[
                                              0])  # pick action with given probability -> Converges fastest
...
def test(self, sess, thread_manager, saver, args):
    ...
    if done and not info:
        ...
        summary = str(datetime.now())[:-7] + "  |  Elapsed: " + time_elapsed + "  |  Count: " + str(
            num_tests) + "  |  Episode Reward: " + str(
            episode_reward) + "  |  Average Reward: " + string_reward_mean
        print(summary)
